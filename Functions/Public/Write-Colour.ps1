function Write-Colour {
    ...
    if ($ColourMatches) {
        Write-Debug "Write-Colour: Found $($ColourMatches.Matches.count) matches"
        $ColourMatches.Matches | ForEach-Object {
            ...
            if ($Colour -Match '[0-9]{1,3},[0-9]{1,3},[0-9]{1,3}') {
                Write-Debug "Write-Colour: $Colour is RGB"
                $RGB = $Colour.split(',')
                $R = $RGB[0]
                $G = $RGB[1]
                $B = $RGB[2]
            ...
            elseif ($Colour -match '#?[a-fA-F0-9]{6}') {
                Write-Debug "Write-Colour: $Colour is hex"
            ...
            # Check colours list
                ...
                if ($null -ne $Colours.$Colour) {
                    Write-Debug "Write-Colour: $Colour is in datastore"
                    $R = $Colours.$Colour.red
                    $G = $Colours.$Colour.green
                    $B = $Colours.$Colour.blue
                }
                else {
                    Write-Warning "Write-Colour: Could not parse colour $Colour"
                }
            }
            ...
            # Add flags, if any
            if ($Flags) {
                Write-Debug "Write-Colour: Flags = $Flags"
        ...
        Write-Output $ParsedInput
    }
    else {
        Write-Warning "Write-Colour: Found no colour matches"
    }
}